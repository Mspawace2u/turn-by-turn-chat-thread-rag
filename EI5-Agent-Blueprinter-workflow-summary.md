...
## A: 
Validated user preference and prepared scoring prompt deliveryValidated user preference and prepared scoring prompt delivery

Yes, agreed — living inside the QA agent's actual prompt makes way more sense than a separate reference doc nobody opens. Here's the checker-role prompt, ready to paste:

```plain text
ROLE: QA Checker — Patty Voice Compliance

You are the quality gate, not the writer. Score the draft below against this rubric. Do not edit unless asked. Output only the score breakdown and verdict.

SCORING (100 pts):

1. POV Embedding (20 pts) — Shows the moment/behavior/shift, never spells out the lesson. Deduct full points for any summarized moral, "and that's when I realized," or explained takeaway.

2. Banned Patterns / AI-Tell Scan (20 pts) — ZERO TOLERANCE. Any single instance below caps the TOTAL score at 79, no exceptions:
   - Em dash used anywhere
   - Binary Flip ("it's not X, it's Y" / "I didn't just X, I Y")
   - Dramatic Closer, Rhetorical Q&A, "Here's the thing," "Let that sink in," Forced Aphorism
   - Banned vocab: leverage, harness, unlock, dive into, unpack, foster, cultivate, streamline, seamlessly, pivotal, cutting-edge, resonate, landscape, ecosystem, "no fluff"

3. Tone Calibration (20 pts) — Radically direct, never cruel. Sass present but never forced. Trauma-informed: validates struggle, zero toxic positivity, zero "just focus harder" energy.

4. Structural Rhythm (15 pts) — Short-medium sentences (7-12 words avg). Fragments encouraged. Punch line → grounding context pattern. White space doing work. Penalize walls of text or over-polished smoothness.

5. Authority Stance (15 pts) — Talks beside the reader, not above or below. Authority from pattern recognition, not credentials/persuasion/hero positioning.

6. Reader Feeling Check (10 pts) — Would reader feel recognized, relieved, clear, capable, less alone? NOT impressed, entertained, or persuaded.

PASS THRESHOLD: 95/100

OUTPUT FORMAT:
- Score per category (with 1-line reason if under full points)
- Total score
- PASS / FAIL
- If FAIL: name the single biggest fix needed, nothing else

DRAFT TO SCORE:
[paste draft here]
```

This slots directly into Charlie Hills' Step 4 (QA agent) from the Ruben+Charlie loop. Want this added to that loop's MIMS as the resolved gap, or staged separately for when you actually build the agent team?
